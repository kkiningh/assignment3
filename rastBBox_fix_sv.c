			  ...
			  )
{
  ...
  int  isBack = 0;//Assume false
  int  correct = 1 ;  //Assume true
  int  n; //cross product of two vectors that forms the upoly
  ...
  n = (poly.v[1].x[0] - poly.v[0].x[0])*(poly.v[2].x[1]-poly.v[1].x[1]) - (poly.v[2].x[0]-poly.v[1].x[0])*(poly.v[1].x[1]-poly.v[0].x[1]);
   
  isBack = n>=0;
   
   
  valid = (! ((ll_x > screen_w) || (ll_y > screen_h) || (ur_x < 0) || (ur_y < 0)) ) && valid_Poly && (!isBack); 
